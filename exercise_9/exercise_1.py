...
for func in ops:
    setattr(tany,func, tany.make_wrapper(func))
for func in rops:
    setattr(tany,func, tany.make_wrapper(func))
for func in number_ops:
    setattr(tany,func, tany.make_wrapper(func))
for func in str_ops:
    setattr(tany,func, tany.make_wrapper(func))
for func in container_ops:
    setattr(tany,func, tany.make_wrapper(func))
for func in container_ops_unchanged:
    if func != "__contains__":
        setattr(tany,func, tany.make_wrapper(func))
    else:
        setattr(tany,func,lambda self,item: item in self.value)